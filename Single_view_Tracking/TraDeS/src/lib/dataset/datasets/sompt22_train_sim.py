...
class SOMPT22_TRAIN_SIM(GenericDataset):
  num_categories = 1
  ...
  def __init__(self, opt, split):
    ...
    print('Using SOMPT22_TRAIN_SIM {}'.format(split))
    ...
    super(SOMPT22_TRAIN_SIM, self).__init__(opt, split, ann_path, img_dir)

    self.num_samples = len(self.images)
    print('Loaded SOMPT22_TRAIN_SIM {} {} samples'.format(split, self.num_samples))
  ...
  def save_results(self, results, save_dir):
    results_dir = os.path.join(save_dir, 'results_sompt22_train_sim')
